...
def index():
    ...
    if request.method == 'POST':
        ...
        if completion == False:
            ...
            aggAccessiDatabase(username, times)
...
def movimenti():
    if request.method == 'POST':
        ...
        date = date.split()
        times = date[3]
        aggCronologiaMovimenti(user_cookie, times, move)
...
if __name__ == '__main__':
    app.run(debug=True, host='localhost')  #host da cambiare, mettere IP dell'alphabot